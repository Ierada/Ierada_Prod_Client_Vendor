...
const AuthModal = ({ onClose, isOpen }) => {
  ...
  const [inputType, setInputType] = useState(""); // "email" or "mobile"
  const [tempToken, setTempToken] = useState("");
  const [isPasswordModalOpen, setIsPasswordModalOpen] = useState(false);

  // Form states
  const [formData, setFormData] = useState({
    identifier: "", // stores either email or mobile
    email: "", // optional email for mobile signup
    mobile: "", // required mobile for email signup
    first_name: "",
    last_name: "",
    password: "",
    confirmPassword: "",
  ...
  const otplessSDKRef = useRef(null);
  ...
  useEffect(() => {
    ...
    script.setAttribute("data-appid", config.VITE_OTPLESS_appId);
    document.head.appendChild(script);
    script.onload = () => {
      otplessSDKRef.current = new window.OTPless();
    };
    return () => {
  ...
  // Timer management - start countdown when OTP modal is shown
  useEffect(() => {
    if (showOtpModal) {
      startOtpTimer();
    }
  ...
  // Function to start/restart the OTP timer
  ...
  // Detect input type and update form
  ...
  // OTP handling functions - memoized to prevent unnecessary re-renders
  ...
  //         // Focus handling moved outside of state update
  ...
  const handleSendOtp = async (type) => {
    ...
    // Clear OTP fields when resending OTP
    setOtp(["", "", "", ""]);

    if (type === "mobile") {
      if (otplessSDKRef.current) {
        try {
          const phoneNumber =
            inputType === "mobile" ? formData.identifier : formData.mobile;
          await otplessSDKRef.current.initiate({
            channel: "PHONE",
            phone: phoneNumber,
            countryCode: "+91",
          });
          setShowOtpModal(true);
          startOtpTimer(); // Restart timer when sending a new OTP
        } catch (error) {
          setErrors((prev) => ({
            ...prev,
  ...
  //     if (verificationInProgress === "mobile") {
  //       const phoneNumber =
  //         inputType === "mobile" ? formData.identifier : formData.mobile;

  //       // 1. Verify with OTPless SDK
  //       const sdkResponse = await otplessSDKRef.current.verify({
  //         channel: "PHONE",
  //         phone: phoneNumber,
  //         countryCode: "+91",
  //         otp: otpCode,
  ...
  //         throw new Error(
  //           sdkResponse.errorMessage || "OTP verification failed"
  //         );
  //       }

  //       let backendResponse;
  //       if (tempToken) {
  //         backendResponse = await verifyMobile({
  //           phone: phoneNumber,
  //           tempToken,
  //         });
  //       } else {
  ...
  //       // 2. Verify with backend API (send raw phone number)
  //       if (backendResponse?.status === 1 && backendResponse.data?.verified) {
  //         setMobileVerified(true);
  //         setShowOtpModal(false);
  //       } else {
  //         throw new Error("Mobile verification not confirmed by backend");
  //       }
  //     }
        ...
        <div className="w-full md:w-1/2 p-8 md:p-12">
          ...
          >
            ...
            <div className="flex justify-center space-x-4 text-sm">
              ...
              {!isSignIn && (
                <a
                  href={`${config.VITE_BASE_WEBSITE_URL}/become-a-seller`}
                  className="border border-[#737A6C] text-[#737A6C] hover:bg-[#737A6C] hover:text-white px-4 py-2 rounded-lg transition duration-200"
                >
                  Register as Vendor
                </a>
